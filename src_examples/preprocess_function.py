...
class MultiClassProcessor(DataProcessor):
    ...
    def get_prototype_examples(self, data_dir):
        test_case_dir = os.path.join(data_dir, '인수계약서_testcase.xlsx')
        test_case = pd.read_excel(test_case_dir)
        return self._create_examples(test_case, 'prototype')
...
class MultiLabelProcessor(DataProcessor):
    ...
    def get_labels(self, data_dir):
        """See base class."""
        labels =[]
        lines = self._read_tsv(os.path.join(data_dir, "all_labels.csv"))
        for line in range(len(lines)):
            labels.append(str(line))
        return labels
...
class NerProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        logger.info("Valid {}".format(os.path.join(data_dir, "valid.tsv", data_dir)))
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "valid.tsv")), "valid", data_dir)
    ...
    def get_prototype_examples(self, data_dir):
        test_case_dir = os.path.join(data_dir, '인수계약서_testcase.xlsx')
        test_case = pd.read_excel(test_case_dir)
        return self._create_examples(test_case, 'prototype', data_dir)

    def get_labels(self, data_dir):
        """See base class."""
        lines = list(pd.read_csv(os.path.join(data_dir, "all_labels.csv"), header=None)[0].values)
        return lines
    ...
    def _create_examples(self, df, set_type, data_dir):
        ...
        examples = []
        if set_type == 'prototype':
            for idx, text in enumerate(df.values):
                guid = "%s-%s" % (set_type, idx)
                text_a = text[0]
                tag = 'O'
                examples.append(InputExample(guid=guid, text_a=text_a, label=tag))
        else :
            labels = pd.read_csv(os.path.join(data_dir, set_type+'_labels.csv'), header=None).values
            for idx, (text, label) in enumerate(zip(df[1].values, labels)):
                guid = "%s-%s" % (set_type, idx)
                text_a = text
                tag = [i for i in label if type(i) == str]
                examples.append(InputExample(guid=guid, text_a=text_a, label=tag))
        return examples
...
def convert_examples_to_features_ner(examples, label_list, max_seq_length, tokenizer):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # label_id = label_map[example.label]
        label_id = [label_map[j] for j in example.label]
        if len(label_id)>max_seq_length :
            pad_label_id = label_id[:max_seq_length]
        else :
            label_padding = [11] * int(max_seq_length-len(label_id))
            pad_label_id = label_id + label_padding
        ...
        if ex_index < 0 :
            ...
            logger.info(
                "segment_ids: %s" % " ".join([str(x) for x in segment_ids]))
            # logger.info("label: %s (id = %d)" % (example.label, label_id))
            logger.info("labels_id : {}".format(pad_label_id))

        features.append(
            InputFeatures(input_ids=input_ids,
                          input_mask=input_mask,
                          segment_ids=segment_ids,
                          label_id=pad_label_id))
    return features
